...
Dadurch entfällt die Abhängigkeit von einer bestimmten Reihenfolge zwischen den beiden Kalibrierschritten: Beide Schritte legen für sich genommen nur eine unabhängige Zahl ab, sodass sich die Referenzpunkte in beliebiger Reihenfolge setzen, beliebig oft wiederholen und – besonders praxisrelevant – auch einzeln nachkalibrieren lassen, etwa wenn nur einer der beiden Anschläge im Feld nachjustiert werden muss, ohne dass der andere neu abgeglichen werden müsste. Reihenfolge-Unabhängigkeit ist dabei nicht dasselbe wie Vollständigkeit: die Umrechnung selbst steht natürlich erst zur Verfügung, sobald beide erforderlichen Referenzpunkte tatsächlich gesetzt wurden – vorher fehlt schlicht die zweite Stütze der Geraden. Der Unterschied zum klassischen Verfahren ist nicht, dass ein Punkt fehlen dürfte, sondern dass es keine Rolle mehr spielt, *welcher* der beiden zuerst gesetzt wird.
